...
void process(std::string path)
{
    unsigned int N;
    int a;
    type T, dt, tau, x_size, y_size, z_size, eps, sigma, k, rmin, temp0, xi;
    bool save;
    std::string par = "Params_full_nvt.txt";
    ...
    Full_nvt_system s(N, x_size, y_size, z_size, eps, sigma, k, temp0, xi);
